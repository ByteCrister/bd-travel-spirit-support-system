...
interface TourReportsDialogProps {
    open: boolean;
...
export function TourReportsDialog({
    open,
    onOpenChange,
    companyId,
    tourId,
    tourTitle,
}: TourReportsDialogProps) {
    ...
    // derive active key from local state instead of store.params
    ...
    useEffect(() => {
        if (!open || !companyId || !tourId) return;
        const key = `${currentPage}-${pageSize}`;
        if (!listCache?.tourReports?.[tourId]?.[key]) {
            fetchReports(companyId, tourId, { page: currentPage, limit: pageSize });
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [open, companyId, tourId, currentPage, pageSize]);
    ...
    // allow changing page even if cachedData is not present
    const handlePageChange = (newPage: number) => {
        // optimistic clamp only if we have server pages info
        if (cachedData?.pages) {
            const safePage = Math.max(1, Math.min(newPage, cachedData.pages));
            setCurrentPage(safePage);
            return;
        }
        // otherwise set requested page and let fetchReports run
        setCurrentPage(Math.max(1, newPage));
    };

    // changing page size should reset page to 1 and refetch
    ...
    const formatReportReason = (reason: REPORT_REASON) => {
        if (!reason) return "";
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="w-full max-w-6xl h-[90vh] flex flex-col p-0 overflow-hidden">
                {/* Header */}
                <div className="px-6 pt-5 pb-4 bg-gradient-to-r from-blue-50 via-indigo-50 to-purple-50 dark:from-blue-950/10 dark:via-indigo-950/10 dark:to-purple-950/10 border-b shrink-0">
                                        ...
                                        >
                                            <CardContent className="p-4">
                                                <div className="space-y-3">
                                                    ...
                                                    {/* Reason */}
                                                    <div className="flex items-center gap-2 bg-orange-50 dark:bg-orange-950/10 px-3 py-2 rounded-lg">
                                                    ...
                                                    <div className="flex flex-wrap items-center gap-2">
                                                        ...
                                                        {report.evidenceCount && report.evidenceCount > 0 && (
                                                            <div className="flex items-center gap-1.5 text-xs bg-blue-50 dark:bg-blue-950/20 text-blue-700 dark:text-blue-300 px-2 py-1 rounded-md">
                ...
                {/* Footer */}
                {cachedData && cachedData.items && cachedData.items.length > 0 && (
                    <div className="px-6 py-2.5 border-t bg-muted/20 shrink-0">
