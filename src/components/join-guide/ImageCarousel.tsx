// ImageCarousel.tsx – supports fill mode and external image URLs
"use client";
...
type ImageCarouselProps = {
  ...
  /** When true, the carousel fills its parent entirely (no aspect‑ratio, absolute positioning) */
  fill?: boolean;
};
...
export default function ImageCarousel({
  ...
  // Build image objects – src is used as‑is (supports absolute URLs or relative paths)
  const images = useMemo(
    () =>
      Images.map((src) => ({
        src,
        alt: src.split("/").pop()?.replace(/[-_]/g, " ") || "carousel image",
      })),
    [Images]
  );
  ...
  // Common image renderer (used in both fill and standard modes)
  const renderImage = (idx: number) => {
    ...
    return (
      <Image
        ...
        alt={images[idx].alt}
        fill
        sizes={fill ? "100vw" : "(max-width: 640px) 100vw, (max-width: 1024px) 80vw, 60vw"}
  ...
  // Fill mode: use absolute positioning to cover the container
  if (fill) {
    return (
      <div className={`relative w-full h-full overflow-hidden ${className ?? ""}`}>
        ...
        {/* Controls and indicators – same as standard mode */}
        {showControls && images.length > 1 && (
          <>
            <div className="absolute inset-0 hidden sm:flex items-center justify-between px-2 md:px-4">
              <button
                aria-label="Previous slide"
                onClick={prev}
                className="pointer-events-auto inline-flex items-center justify-center rounded-full bg-white/60 hover:bg-white/80 dark:bg-black/50 dark:hover:bg-black/70 p-1.5 sm:p-2 md:p-2.5 text-gray-800 dark:text-white shadow-md backdrop-blur-sm transition-all duration-200 focus-visible:ring-2 focus-visible:ring-white/50 touch-manipulation active:scale-95"
              >
  ...
  // Standard mode (with aspect‑video)
  return (
