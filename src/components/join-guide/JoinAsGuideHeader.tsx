...
const JoinAsGuideHeader = () => {
    ...
    const { baseUser, baseMeta, fetchBaseUser } = useCurrentUserStore();
    const isLoggedIn = !!baseUser;
    const isLoading = baseMeta.loading || baseMeta.inFlight;
    ...
    return (
        <motion.header
            ...
            className="sticky top-0 z-50 border-b border-gray-200/50 dark:border-gray-700/50 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl supports-[backdrop-filter]:bg-white/60 dark:supports-[backdrop-filter]:bg-gray-900/60"
            style={{
        ...
        >
            ...
            >
                ...
                <div className="flex items-center gap-2 sm:gap-3 md:gap-4 flex-shrink-0">
                    {/* Conditional Button: Loading, Dashboard, or Login */}
                    {isLoading ? (
                        // Loading State with dots animation
                        <motion.button
                            initial={{ opacity: 0, x: 20 }}
                            animate={{ opacity: 1, x: 0 }}
                            transition={{ duration: 0.6, ease: "easeOut", delay: 0.1 }}
                            disabled
                            className="group relative inline-flex items-center justify-center gap-1.5 sm:gap-2.5 rounded-xl sm:rounded-2xl 
                        ...
                        >
                            ...
                            <div className="absolute inset-0 rounded-xl sm:rounded-2xl bg-gradient-to-r from-emerald-500/20 via-teal-500/20 to-cyan-500/20 opacity-30"></div>

                            {/* Icon Container */}
                            <div className="relative z-10 flex items-center justify-center flex-shrink-0">
                                <div className="flex h-5 w-5 sm:h-6 md:h-7 sm:w-6 md:w-7 items-center justify-center rounded-md sm:rounded-lg bg-gradient-to-br from-emerald-500/10 to-cyan-500/10 border border-emerald-200/30 dark:border-emerald-500/20">
                                    <FiUser className="h-2.5 w-2.5 sm:h-3 md:h-3.5 sm:w-3 md:w-3.5 text-emerald-600 dark:text-emerald-400" />
                                </div>
                            </div>

                            {/* Text Content with Loading Dots */}
                            <div className="relative z-10 flex flex-col items-start min-w-0">
                                <span className="text-xs sm:text-sm font-semibold bg-gradient-to-r from-gray-700 to-gray-600 dark:from-gray-200 dark:to-gray-300 bg-clip-text text-transparent">
                                    <span className="ml-1 flex items-center space-x-0.5">
                                        <motion.span
                                            animate={{ opacity: [0, 1, 0] }}
                                            transition={{ duration: 1.5, repeat: Infinity, delay: 0 }}
                                            className="inline-block h-1 w-1 rounded-full bg-emerald-600 dark:bg-emerald-400"
                                        />
                                        <motion.span
                                            animate={{ opacity: [0, 1, 0] }}
                        ...
                        // Dashboard Button (when user is logged in)
                        <motion.button
                            initial={{ opacity: 0, x: 20 }}
                            animate={{ opacity: 1, x: 0 }}
                            transition={{ duration: 0.6, ease: "easeOut", delay: 0.1 }}
                            ...
                            className="group relative inline-flex items-center gap-1.5 sm:gap-2.5 rounded-xl sm:rounded-2xl 
                                ...
                                border border-emerald-400/30 dark:border-emerald-500/30 backdrop-blur-sm
                                px-2.5 sm:px-3 md:px-4 py-2 sm:py-2.5 md:py-3 text-xs sm:text-sm font-semibold text-white
                                shadow-md sm:shadow-lg shadow-emerald-500/25 dark:shadow-emerald-900/50
                                hover:shadow-lg sm:hover:shadow-xl hover:shadow-emerald-500/40 dark:hover:shadow-emerald-800/60
                                hover:border-emerald-300/50 dark:hover:border-emerald-400/50
                                transition-all duration-300
                                focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 
                                focus-visible:ring-offset-2 focus-visible:ring-offset-transparent
                                overflow-hidden"
                        >
                            {/* Background Glow Effect */}
                            <div className="absolute inset-0 rounded-xl sm:rounded-2xl bg-gradient-to-r from-emerald-500/40 via-teal-500/40 to-cyan-500/40 opacity-0 transition-opacity duration-300 group-hover:opacity-100"></div>

                            {/* Icon Container */}
                            <div className="relative z-10 flex items-center justify-center flex-shrink-0">
                                <div className="flex h-5 w-5 sm:h-6 md:h-7 sm:w-6 md:w-7 items-center justify-center rounded-md sm:rounded-lg bg-white/20 border border-white/30 group-hover:border-white/50 transition-all duration-300">
                                    <FiHome className="h-2.5 w-2.5 sm:h-3 md:h-3.5 sm:w-3 md:w-3.5 text-white transition-all duration-300 group-hover:scale-110" />
                                </div>
                            </div>

                            {/* Text Content */}
                            <div className="relative z-10 flex flex-col items-start min-w-0">
                                <span className="text-xs sm:text-sm font-semibold text-white group-hover:text-white/90 transition-colors duration-300 truncate">
                                    Dashboard
                                </span>
                                <div className="h-0.5 w-0 bg-gradient-to-r from-white to-emerald-200 rounded-full transition-all duration-300 group-hover:w-full"></div>
                            </div>

                            {/* Shine Effect */}
                            <div className="absolute inset-0 rounded-xl sm:rounded-2xl bg-gradient-to-r from-transparent via-white/20 to-transparent opacity-0 -translate-x-full transition-all duration-700 group-hover:opacity-100 group-hover:translate-x-full"></div>
                        </motion.button>
                    ) : (
                        // Original Login Button (when no user is logged in)
                        <motion.button
                            initial={{ opacity: 0, x: 20 }}
                            animate={{ opacity: 1, x: 0 }}
                            transition={{ duration: 0.6, ease: "easeOut", delay: 0.1 }}
