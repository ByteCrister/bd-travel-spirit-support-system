...
export function DestinationBlockForm({
    ...
    const destinations: DestinationBlockFormTypes[] = (values.destinations ?? []).map((d) => ({
        ...
        attractions: (d.attractions ?? []).map((a) => ({
            ...
            images: (a.images ?? []).filter((img): img is string => Boolean(img)),
                        ...
                        <AccordionContent className="space-y-6 pb-4">
                            ...
                            <div className="space-y-3">
                                ...
                                <ImageUploader
                                    label=""
                                    multiple
                                    value={d.images?.filter((img): img is string => Boolean(img)) ?? []}
                                    onChange={(images) =>
                                        updateAt(idx, {
                                            images: Array.isArray(images)
                                                ? images.filter((i): i is string => Boolean(i))
                                                : images
                                                    ? [images]
                                                    : [],
                                        })
                                    ...
                                    {d.attractions.map((a, ai) => (
                                        <div key={ai} className="rounded-lg border bg-muted/50 p-4 space-y-3">
                                            ...
                                            <ImageUploader
                                                ...
                                                multiple
                                                value={a.images?.filter((img): img is string => Boolean(img)) ?? []}
                                                onChange={(images) =>
                                                    updateAttraction(idx, ai, {
                                                        images: Array.isArray(images)
