// stores/stripe-payment-account.store.ts
...
export const usePaymentAccountStore = create<Store>()(
    devtools((set, get) => ({
        ...
        createStripePaymentMethod: async (payload: CreateStripePaymentMethodDTO) => {
            ...
            // UPDATED: optimistic object uses stripeMeta top-level (matches updated types)
        ...
        updateAccount: async (id: string, payload: PaymentAccountUpdateDTO) => {
            ...
            // Merge payload into prev for optimistic update.
            // Note: payload may include stripe fields; keep shape consistent with PaymentAccount.
            const optimisticMerge = { ...prev, ...payload, updatedAt: new Date().toISOString() } as PaymentAccount;
