...
async function approveRequestHandler(
    ...
    const updatedRequest = await withTransaction(async (session) => {
        ...
        await requestWithGuide.approve(new Types.ObjectId(adminObjectId), {
            session,
        });
    ...
    if (sendEmail && userEmail) {
        try {
            const html = guideUpdatePasswordHtml(userEmail, updatedPass);
            await mailer(userEmail, "Guide new password", html);
        } catch (emailError) {
            console.error("Failed to send approval email:", emailError);
            // Request succeeded, password updated – just email delivery failed
        }
    }
