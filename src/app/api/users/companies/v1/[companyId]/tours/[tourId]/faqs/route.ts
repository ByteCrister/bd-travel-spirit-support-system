...
import { GetTourFaqsResponse } from "@/types/tour/tour-detail-faqs.types";
...
export class FAQResponseService {
  static async getTourFAQs(
    ...
    const baseMatch = buildBaseQuery(tourObjectId, params);
    ...
    const sortKey =
      finalSortField === "likes"
        ? "likesCount"
        : finalSortField === "dislikes"
          ? "dislikesCount"
          : finalSortField;
    ...
    // If search term is provided, add a $match stage that looks in question, answer, and asker name
    if (params.search) {
...
async function getFAQsHandler(
  ...
  await ConnectDB();
  const data = await FAQResponseService.getTourFAQs(
    tourId.toString(),
    queryParams,
  );
