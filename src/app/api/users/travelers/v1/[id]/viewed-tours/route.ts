...
import { TravelerViewedTour, PaginatedResponse } from '@/types/user/traveler.types';
...
interface ViewedTourAggregationResult {
    tour: {
...
async function getTravelerViewedTours(
    req: NextRequest,
    { params }: Params
): Promise<HandlerResult<PaginatedResponse<TravelerViewedTour>>> {
    await ConnectDB();
    ...
    const pipeline: PipelineStage[] = [
        ...
        { $unwind: '$viewedTours' },
        { $sort: { 'viewedTours.viewedAt': -1 as const } },
        {
            $facet: {
                ...
                data: [
                    ...
                    { $unwind: '$tour' },
                    {
                        $project: {
                            ...
                            viewedAt: '$viewedTours.viewedAt',
                            durationSeconds: '$viewedTours.durationSeconds',
                        },
                    },
                ],
            },
    ...
    const data = (result[0]?.data || []) as ViewedTourAggregationResult[];

    const viewedTours: TravelerViewedTour[] = data.map((item) => ({
        tour: {
    ...
    const responseData: PaginatedResponse<TravelerViewedTour> = {
        data: viewedTours,
        total,
        page,
        limit,
...
export const GET = withErrorHandler(getTravelerViewedTours);